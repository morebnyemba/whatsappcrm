...
class GetReferralSettingsConfig(BasePydanticConfig):
    ...
    if step.step_type == 'send_message':
        ...
        try:
            ...
            for i, action_item_conf in enumerate(action_step_config.actions_to_run):
                ...
                elif action_type == ActionType.GET_REFERRAL_SETTINGS:
                    if not REFERRALS_ENABLED:
                        logger.error(f"Step '{step.name}': 'get_referral_settings' action called, but referrals app not available.")
                        current_step_context[action_item_root.output_variable_name] = {}
                        continue
...
def _trigger_new_flow(contact: Contact, message_data: dict, incoming_message_obj: Message) -> List[Dict[str, Any]]:
    ...
    #
    # NOTE: football_data_app/bet_flow_handler.py's own comments (MENU_ITEMS,
    # module docstring) claim every dynamic dropdown beyond BET_MENU never
    # submitted a value on any device tested. That history is stale — the user
    # has confirmed on a real device that the full flow (menu -> browse ->
    # markets -> outcomes -> stake -> confirm -> placement) works end to end.
    # The real, confirmed gap is that BET_BROWSE's fixture dropdown has no
    # paging: _fixtures_options() takes a flat MAX_FLOW_OPTIONS-sized slice
    # with no way to reach fixtures beyond it. See the pagination added to
    # _browse_screen()/_fixtures_options() in bet_flow_handler.py.

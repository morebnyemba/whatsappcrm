import logging
from typing import Optional, Dict, List # List is still used for internal types, but not return type
...
# Removed WHATSAPP_CHAR_LIMIT and MESSAGE_SEPARATOR as we are no longer splitting messages
...
def get_formatted_football_data(
    ...
    days_ahead: int = 10, 
    days_past: int = 2
) -> str: # Reverted return type to str for single message output
    """
    ...
    now = timezone.now()
    message_lines: List[str] = [] # Build message in parts, then join at the end
    ...
    if data_type == "scheduled_fixtures":
        ...
        # --- IMPORTANT: Strict limit on matches to ensure single message fits ---
        # Adjust this number based on average match detail length and char limit
        num_matches_to_display = 7 # Displaying up to 7 matches for conciseness
        ...
        for match in fixtures_qs[:num_matches_to_display]: 
            # Compact line output for scheduled matches
            line = f"\n*{match.league.name}* (ID: {match.id})" # League and Match ID on one line
            ...
            # --- Prepare compact odds line ---
            compact_odds_parts: List[str] = []

            # H2H (Moneyline) - H:Odds | D:Odds | A:Odds
            if 'h2h' in aggregated_outcomes:
                ...
                if home_odds: compact_odds_parts.append(f"H:*{home_odds.odds:.2f}*") # Formatted to 2 decimal places
                if draw_odds: compact_odds_parts.append(f"D:*{draw_odds.odds:.2f}*")
                if away_odds: compact_odds_parts.append(f"A:*{away_odds.odds:.2f}*")

            # Totals (Over/Under) - O#.##:Odds | U#.##:Odds
            if 'totals' in aggregated_outcomes:
                ...
                if best_overall_over: compact_odds_parts.append(f"O{best_overall_over.point_value if best_overall_over.point_value is not None else ''}:*{best_overall_over.odds:.2f}*")
                if best_overall_under: compact_odds_parts.append(f"U{best_overall_under.point_value if best_overall_under.point_value is not None else ''}:*{best_overall_under.odds:.2f}*")
            ...
            if 'spreads' in aggregated_outcomes:
                ...
                if best_home_spread: compact_odds_parts.append(f"S {match.home_team.name}({best_home_spread.point_value}):*{best_home_spread.odds:.2f}*")
                if best_away_spread: compact_odds_parts.append(f"S {match.away_team.name}({best_away_spread.point_value}):*{best_away_spread.odds:.2f}*")
        ...
        for match in fixtures_qs[:num_matches_to_display]: 
            # Compact line output for finished results
            line = f"\n*{match.league.name}* (ID: {match.id})" # League and Match ID
            line += f"\n{match.home_team.name} vs {match.away_team.name}"

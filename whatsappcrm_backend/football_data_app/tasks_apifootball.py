...
def _process_apifootball_odds_data(fixture: FootballFixture, odds_data: dict):
    ...
    def _to_odds(raw):
        """Decimal(str(raw)) raises decimal.InvalidOperation for values like
        '', '-', or 'N/A' -- a subclass of ArithmeticError, not ValueError, so
        it wouldn't be caught by a bare `except (ValueError, TypeError)`.
        Returns None for anything unparsable so the caller can skip just that
        one outcome instead of aborting the whole fixture."""
        try:
            return Decimal(str(raw))
        except (ArithmeticError, ValueError, TypeError):
            logger.warning(f"Skipping unparsable odd '{raw}' for fixture {fixture.id}")
            return None
    ...
    for bookmaker_data in odds_data.get('odd_bookmakers', []):
        ...
        # This bookmaker's h2h market for this fixture, if this refresh still
        # offers it -- used below to deactivate a market the bookmaker has
        # dropped entirely, which would otherwise stay is_active=True forever
        # with odds that can never change again.
        market_still_offered = False
        ...
        for odds_entry in bookmaker_data.get('bookmaker_odds', []):
            ...
            if odd_1 or odd_x or odd_2:
                market_still_offered = True
                # Create H2H market
                category, _ = MarketCategory.objects.get_or_create(name='Match Winner')
                
                # Update the existing market/outcomes in place rather than deleting and
                # recreating them. Market -> MarketOutcome -> Bet are all
                # on_delete=CASCADE, so deleting a Market here cascade-deletes every
                # MarketOutcome under it, which cascade-deletes any Bet a user already
                # placed against one of those outcomes -- see the identical fix and
                # full explanation in tasks_api_football_v3.py's
                ...
                odds_value = _to_odds(odd_1) if odd_1 else None
                if odds_value is not None:
                    outcome = upsert_market_outcome(market, fixture.home_team.name, None, odds_value)
                    seen_outcome_ids.add(outcome.id)

                odds_value = _to_odds(odd_x) if odd_x else None
                if odds_value is not None:
                    outcome = upsert_market_outcome(market, 'Draw', None, odds_value)
                    seen_outcome_ids.add(outcome.id)

                odds_value = _to_odds(odd_2) if odd_2 else None
                if odds_value is not None:
                    outcome = upsert_market_outcome(market, fixture.away_team.name, None, odds_value)
                    seen_outcome_ids.add(outcome.id)

                if seen_outcome_ids:
                    total_outcomes_created += len(seen_outcome_ids)
                    logger.debug(f"Upserted market with {len(seen_outcome_ids)} outcomes for bookmaker {bookmaker_name}")

                market.outcomes.exclude(id__in=seen_outcome_ids).update(is_active=False)

        if not market_still_offered:
            # This bookmaker no longer returned any odds at all for this
            # fixture in this refresh -- deactivate its h2h market rather
            # than leaving it active with stale odds forever.
            Market.objects.filter(
                fixture=fixture, bookmaker=bookmaker, api_market_key='h2h', is_active=True
...
@shared_task(name="football_data_app.tasks_apifootball.run_apifootball_full_update", queue='football_io')
...
@shared_task(name="football_data_app.tasks_apifootball._prepare_and_launch_event_odds_chord", queue='football_io')
...
@shared_task(bind=True, queue='football_io')
def dispatch_odds_fetching_after_events_task(self, results_from_event_fetches):
    ...
    # Fixtures needing odds, with the day they fall on. apifootball's get_odds
    # filters by date range and returns every match's odds for that window in one
    # request, so we fetch odds in bulk per day instead of one request/fixture.
    ...
    logger.info(f"Found {fixture_count} fixtures requiring odds updates across {len(days)} day(s)")

    tasks = [fetch_odds_for_date_task.s(day) for day in sorted(days)]
    group(tasks).apply_async()
    logger.info(f"Dispatched {len(tasks)} bulk odds task(s) for {fixture_count} fixtures "
                f"(was 1 task/fixture; now 1 task/day).")
    logger.info("="*80)
...
def fetch_odds_for_date_task(self, date_str: str):
    """
    Fetch odds in bulk for every tracked fixture on a given day (apifootball.com).

    apifootball's get_odds accepts a date range (from/to) and returns odds for all
    matches in that window in a single request, each tagged with its match_id.
    This replaces one request per fixture with one request per day, then reuses
    the exact per-match processing so behaviour is unchanged.
    """
    jitter_delay = random.uniform(0.5, 2.0)
    time.sleep(jitter_delay)
    logger.info(f"TASK START: fetch_odds_for_date_task - date={date_str}")

    try:
        client = APIFootballClient()
        odds_list = client.get_odds(date_from=date_str, date_to=date_str)
        if not isinstance(odds_list, list):
            odds_list = []
        logger.info(f"Bulk odds returned {len(odds_list)} item(s) for {date_str}")

        # Group by match_id (first element per match mirrors get_match_odds()[0]).
        by_match = {}
        for element in odds_list:
            if isinstance(element, dict):
                mid = element.get('match_id')
                if mid is not None:
                    by_match.setdefault(str(mid), element)

        if not by_match:
            logger.info(f"No odds available for {date_str}.")
            return {"date": date_str, "status": "no_odds", "fixtures": 0}

        fixtures = FootballFixture.objects.filter(api_id__in=list(by_match.keys()))
        processed = 0
        for fixture in fixtures:
            element = by_match.get(str(fixture.api_id))
            if not element:
                continue
            try:
                with transaction.atomic():
                    fixture_for_update = FootballFixture.objects.select_for_update().get(id=fixture.id)
                    _process_apifootball_odds_data(fixture_for_update, element)
        ...
        logger.info(f"TASK END: fetch_odds_for_date_task - processed odds for {processed} fixture(s)")
        return {"date": date_str, "status": "success", "fixtures": processed}

    except APIFootballException as e:
        logger.error(f"apifootball API error during bulk odds fetch ({date_str}): {e}")
        raise self.retry(exc=e)
    except Exception as e:
        logger.error(f"Unexpected error during bulk odds fetch ({date_str}): {e}", exc_info=True)
        raise self.retry(exc=e)
...
@shared_task(name="football_data_app.tasks_apifootball.run_score_and_settlement_task", queue='football_io')
...
@shared_task(name="football_data_app.tasks_apifootball.process_ticket_settlement_task", queue='football_io')
...
@shared_task(name="football_data_app.tasks_apifootball.process_ticket_settlement_batch_task", queue='football_io')
...
@shared_task(bind=True, name="football_data_app.tasks_apifootball.reconcile_and_settle_pending_items", queue='football_io')
...
@shared_task(name="football_data_app.tasks_apifootball.settle_fixture_pipeline", queue='football_io')
...
@shared_task(bind=True, max_retries=3, default_retry_delay=120, queue='football_io')
...
@shared_task(name="football_data_app.run_apifootball_full_update", queue='football_io')
...
@shared_task(name="football_data_app.run_score_and_settlement_task", queue='football_io')
